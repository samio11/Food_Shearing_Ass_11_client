...
const MyAddedFoodManage = () => {
    ...
    const getData = async () => {
        const { data } = await axios(`${import.meta.env.VITE_BACKEND_URL}/manage_food/${user?.email}`, { withCredentials: true });
        try {
            setTotalData(data)
        }
        catch (error) {
    ...
    const deleteFood = async (id) => {
        ...
        Swal.fire({
            title: "Are you sure?",
            text: "The Food you selected is permanently deleted",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                const { data } = axios.delete(`${import.meta.env.VITE_BACKEND_URL}/my_food/${id}`)
    ...
    return (
        <div>
            <Helmet>
                <title>ShareByte | My Added Food</title>
            </Helmet>
                        ...
                        {/* row 1 */}
                        {
                            totalData?.map((x, idx) => {
                                return (
                                    <tr key={idx}>
                                        <td>{x.Food_Name}</td>
                                        <td><img className='w-[70px] h-[70px] rounded-full' src={x.Food_Image} alt={x.Food_Name} /></td>
                                        <td>{x.Food_Quantity}</td>
                                        <td>{x.Pickup_Location}</td>
                                        <td>{x.Expired_Date_Time}</td>
                                        <td>{x.Additional_Notes}</td>
                                        <td><button onClick={() => deleteFood(x._id)}><MdDelete /></button></td>
                                        <td><Link to={`/update_food_manage/${x._id}`}><CiEdit/></Link>
                                        </td>
                                    </tr>
                                )
                            })
...
export default MyAddedFoodManage;